...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        await getRemoteData(url + '/api/Alexa/GetUserName' + queryParams)  
          .then(async (response) => {  
              if (response !== ""){
                speakOutput = 'Hola ' + response + ". ¿Qué necesitas? Recuerda que puedes pedirme que te cree una dieta o un entrenamiento para hoy.";
              } else {
                  speakOutput = 'Bienvenido. Antes de iniciar necesito conocer algunas cosas sobre tí para hacer todo mucho más preciso. Empecemos por tu nombre. ¿Cómo te llamas?';
              }
          })  
          .catch((err) => {  
...
const GetDietTypesIntentHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Los tipos de dietas que existen son: carnívora, omnívora, paleo, vegana, vegetariana y mediterránea';
...
const GetTrainsIntentHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Los tipos de entrenamientos que puedes realizar son: pierna, glúteos, espalda, pecho, brazo y cardio';
...
const UpdateUserDietTypeIntentHandler = {
  ...
  handle(handlerInput) {
    const userId = Alexa.getUserId(handlerInput.requestEnvelope);
    const dietType = Alexa.getSlotValue(handlerInput.requestEnvelope, 'dietType');
...
const UpdateUserObjectiveIntentHandler = {
  canHandle(handlerInput) {
    return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
      && Alexa.getIntentName(handlerInput.requestEnvelope) === 'UpdateUserObjective';
  },

  handle(handlerInput) {
    const userId = Alexa.getUserId(handlerInput.requestEnvelope);
    const objective = Alexa.getSlotValue(handlerInput.requestEnvelope, 'objective');
...
const postRemoteData = (url, data = {}) => new Promise((resolve, reject) => {
  const client = url.startsWith('https') ? require('https') : require('http');
  const options = {
    method: 'POST',
    headers: {
...
const FallbackIntentHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Lo siento, desconozco sobre el tema. Por favor, inetntalo de nuevo.';
...
const ErrorHandler = {
    ...
    handle(handlerInput, error) {
        const speakOutput = 'Lo siento, he tenido problemas para resolver lo que me has pedido. Por favor, intentalo de nuevo.';
